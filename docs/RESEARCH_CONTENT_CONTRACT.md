...
## 1. 城市研究索引 `SHENYANG_RESEARCH_INDEX.json`（→ `<city>/index.json`）
...
```jsonc
{
  ...
  "methodology_notes": ["…"],
  "red_lines": ["…"],
  "source_of_truth": { "research_doc": "SHENYANG_RESEARCH.md", "process_reports": "…", "lineage": "outputs/results_registry.csv" }
}
```
...
1. `id` 必须与 `SHENYANG_RESEARCH.md` 中 `## <id> …` 的标题一致，前端据此切分原文。
...
## 2. 研究正文 `SHENYANG_RESEARCH.md`

- 按 `## <id> <标题>` 分节；节内固定使用 `### 研究问题 / 为什么研究 / 数据 / 方法 / 结果 / 结论 / 证据等级 / 局限 / 前端一句话`。
- 前端按研究点读取对应章节作"查看原文"，不重排、不改写。
- 表格用标准 Markdown 表格，图片用 `![alt](outputs/figures/Fxx.png)`。

## 3. 交互数据表（`outputs/tables/*.csv`）
